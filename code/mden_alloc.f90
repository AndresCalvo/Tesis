...
module declare
  ...
  real(dp), parameter     ::  hbar = 1.0545718E-34_dp      ! reduced Planck's constant hbar = h/(2*pi)
  ...
  real(dp), parameter     :: Rmax = 2.0_dp            ! radius of the waveguide
  real(dp), parameter     :: Lmax = 1.0_dp              ! size of z step
  integer(i4b), parameter :: Nt = 256                   !size of T window
  integer(i4b), parameter :: Nr = 1000 + 1 ! steps in r
  integer(i4b), parameter :: Nz = 100 + 1   ! steps in z

  real(dp), parameter     :: dt = 2.0_dp * Tmax / (Nt - 1)    ! time step size
  real(dp), parameter     :: dr = Rmax / (Nr - 1)               ! size of r step
  real(dp), parameter     :: dz = Lmax / (Nz - 1)
  real(dp), parameter     :: resonance_tol = 0.5_dp * pi / Tmax   ! ≈ 0.1571
  ...
  real(dp), parameter     :: v_g = omega0 / k0           ! group velocity (normalized units: omega0=1, k0=1)
  ...
  real(dp), parameter, dimension(7) :: FrequencyLevels = EnergyLevels * 1.602176634E-19_dp / hbar   ! energy frequency levels [rad/s]: eV -> J -> rad/s
  real(dp), parameter, dimension(7) :: Delta = FrequencyLevels  ! frequency shifts
  ! coordinate grids
  real(dp), allocatable :: t(:), freq(:)       ! time, space and frequency domains
  real(dp), dimension(Nz) :: z(Nz)             ! z (propagation direction) domain
  real(dp), allocatable :: r(:)                ! radial domain
...
! physical variables (field, envelope, density matrix elements, etc)
  complex(cdp), allocatable :: Afield(:,:), Afieldp1(:,:), Afield_f(:,:), Afield_fp1(:,:), Pfield_f(:,:)
  complex(cdp), allocatable :: rho(:,:,:,:), rho0(:,:,:,:)
  complex(cdp), allocatable :: initial_a_t(:)
  complex(cdp), allocatable :: tdata(:), diff(:), sol(:)
  complex(cdp), allocatable :: Pfield(:,:)
end module declare
...
module globals
  ...
  contains
    ...
    subroutine complex_to_real(arr, m, res)
      implicit none
      integer(i4b), intent(in) :: m
      complex(dp), intent(in) :: arr(m)
      real(dp), intent(out) :: res(2 * m)
      integer i

      do i=1, m
        res(2 * i - 1) = real(arr(i))
        res(2 * i) = aimag(arr(i))
      end do

    end subroutine complex_to_real
    !_______________________________________________________________
    subroutine real_to_complex(arr, m, res)
      implicit none
      integer(i4b), intent(in) :: m
      real(dp), intent(in) :: arr(2 * m)
      complex(cdp), intent(out) :: res(m)
      integer ::  i

      do i=1, m
        res(i) = cmplx(arr(2 * i - 1), arr(2 * i), kind=cdp)
      end do
...
subroutine setup !sets range arrays for time and space coordinates
  ...
  allocate(t(Nt), freq(Nt))
  allocate(r(Nr))
  allocate(Afield(Nt,Nr), Afield_f(Nt,Nr), Afield_fp1(Nt,Nr), Pfield_f(Nt,Nr))
  allocate(rho(Nt,Nr,7,7))
  allocate(initial_a_t(Nt))
  !allocate(tdata(Nt), diff(Nt), sol(Nt))
  allocate(Pfield(Nt,Nr))
...
program main
  ...
  use globals
  implicit none
  integer :: i, ii, j, count
  integer(i4b) :: t_start, t_end, rate, res_flag
  real(dp), allocatable :: temp1(:), temp2(:)

  allocate(temp1(2*Nt), temp2(2*Nt))
  call setup
  write(*, *) 'Finished setup'
  write(*, *) 'Starting density matrix evolution'
  open(9, file='start.dat')
  open(10, file='results.dat')
  open(11, file='start_f.dat')
  ...
  do i=1, Nt
    do ii=1, Nr
      count = mod(ii, 10)
      if (count == 1) then
        write(9, *) t(i), r(ii), real(Afield(i, ii))**2 + aimag(Afield(i, ii))**2
      end if
    end do
    write(9, *)
  end do

  !write initial data in fourier space
  call fourier_transform(Afield, 1, Nt, Nr)
  do i=1, Nt
    do ii=1, Nr
      count = mod(ii, 10)
      if (count == 1) then
        write(11, *) freq(i), r(ii), real(Afield(i, ii))**2 + aimag(Afield(i, ii))**2
      end if
    end do
    write(11, *)
  end do
  ...
  do i=1, Nz !main loop over z
    write(*, *) 'Comenzando loop num: ', i
    count=mod(i, 5)
    if (count == 1) then !loop for writing into file
      do j=1, Nt
        write(10, *) t(j), z(i), abs(Afield(j, 1))
      end do
      write(10, *)
    end if
    ...
    do ii=1, Nr
      ...
      call complex_to_real(Pfield(:, ii), Nt, temp1)
      call four1(temp1, Nt, 1)
      call real_to_complex(temp1, Nt, Pfield_f(:, ii))

      call complex_to_real(Afield(:, ii), Nt, temp2)
      call four1(temp2, Nt, 1)
      call real_to_complex(temp2, Nt, Afield_f(:, ii))

    end do

    call PropagationStep(Afield_f, Pfield_f, Afield_fp1, res_flag)
    ...
    do ii=1, Nr
      ...
      call complex_to_real(Afield_fp1(:, ii), Nt, temp2)
      call four1(temp2, Nt, -1)
      call real_to_complex(temp2, Nt, Afield(:, ii))
      Afield(:, ii) = Afield(:, ii) / real(Nt, dp)
  ...
  write(*, *) 'Tiempo total transcurrido: ', real(t_end - t_start)/real(rate), 'segundos'
end program main
...
subroutine matrixDensityEvol(a, r) !solves for rho in time with A field as input
  ...
  complex(cdp), allocatable, save :: rabi(:,:,:,:)
  integer :: i
  real(dp) :: Tr12, Tr16, Tr23, Tr24, Tr25, Tr27, Tr36, Tr46, Tr56, Tr67

  if (.not. allocated(rabi)) allocate(rabi(Nt, Nr, 7, 7))
...
subroutine fourier_transform(data, isign, nt, nr) !wrapper for transform on arrays of form f(t, r)
  use globals, only: real_to_complex, complex_to_real
  implicit none
  double complex data(nt, nr)
  double precision temp(2*nt)
  integer isign, nr, nt, i
  
  
  do i=1, nr
    call complex_to_real(data(:, i), nt, temp)
    call four1(temp, nt, isign)
    call real_to_complex(temp, nt, data(:, i))
  end do
...
subroutine TotalPolarization(rho, pol) ! takes rho (density matrix) to calculate total polarization
  ...
  implicit none
  complex(cdp), dimension(Nt, Nr, 7, 7), intent(in) :: rho
...
subroutine RightHandFieldTerms(FAfield, omega, RHFT)
  ...
  implicit none
  integer(i4b) :: i
  complex(cdp), allocatable, save :: perpFA(:,:) ! radial laplacian of FAfield
  ...
  if (.not. allocated(perpFA)) allocate(perpFA(Nt, Nr))
...
!_________________________________________________________________
subroutine PropagationStep(FA_n, FP_n, FA_np1, res_flag)
  use nrtype
  use declare, only : dt, Nt, Nr, im, dz, v_g, k0, freq, resonance_tol
  implicit none
  ...
  integer(i4b), intent(out) :: res_flag !flag for resonance
  complex(cdp), allocatable, save :: RHFT(:,:), RHPT(:,:)
  real(dp), allocatable, save :: omega(:)
  integer(i4b) :: i

  if (.not. allocated(RHFT)) allocate(RHFT(Nt, Nr), RHPT(Nt, Nr))
  if (.not. allocated(omega)) allocate(omega(Nt))
  omega = freq  ! use frequency grid from declare module
  call RightHandFieldTerms(FA_n, omega, RHFT)
  call PolarizationTerms(FP_n, omega, RHPT)

  res_flag = 0
  do i=1, Nt
    if (abs(k0 * v_g - omega(i)) .le. resonance_tol) then
      write(*, *) 'Resonance hit at freq.: ', omega(i)
      res_flag = 1
      FA_np1(i, :) = FA_n(i, :)
      cycle
    end if
...
subroutine PolarizationTerms(FPField, omega, RHPT)
  ...
  real(dp), dimension(Nt),intent(in) :: omega ! frequency space grid
