...
module declare
  ...
  real(dp), parameter     :: dt = 2.0_dp * Tmax / Nt            ! time step size
...
! physical variables (field, envelope, density matrix elements, etc)
  complex(cdp), dimension(Nt, Nr) :: Afield, Afieldp1, Afield_f, Afield_fp1, Pfield_f
  complex(cdp), dimension(Nt, Nr, 7, 7) :: rho, rabi, rho0
...
module globals
  ...
  contains
    ...
    !___________________________________________________________________
    function mat_to_vec(mat, vec) !specifically designed for this 7 level matrix with 10 allowed transitions
      use declare
      complex(cdp), intent(in) :: mat(Nt, Nr, 7, 7)
      real(dp), intent(out) :: vec(Nt, Nr, 27)
      implicit none
  ...
  do i=1, Nz !main loop over z
    ...
    if (count == 0) then !loop for writing into file
      do j=1, Nt
        ...
        !write(10, *) t(j), z(i), real(Afield(j, 1))**2 + aimag(Afield(j, 1))**2
      end do
      write(10, *)
    end if
    ...
    call PropagationStep(Afield_f, Pfield_f, Afield_fp1)

    do ii=1, Nr
      temp2 = complex_to_real(Afield_fp1(:, ii), Nt)
      call four1(temp2, Nt, -1)
